...
def generate_mines(mine_free, dir):
    ...
    # initialize board with answers, which is a dictionary
    board_ans = {}
    for i in range(9):
        for j in range(9):
    ...
    for i in range(9):
        for j in range(9):
            ...
            for dir_x, dir_y in dir:
                ...
                coor_y = j + dir_y
                if 0 <= coor_x < 9 and 0 <= coor_y < 9:
                    if board_ans[str(coor_x) + str(coor_y)] == "X":
                        sum_mines += 1
            # replace total mines to initial value
            board_ans[str(i) + str(j)] = str(sum_mines)
...
(ex: a5). to add or remove a flag, add 'f' to the cell (ex: a5f) \n")
    ...
    # flag case  
    if board[str(p1) + str(p2)] == "F":
        show_board(board)
        print("\nThere is a flag there")
    
    # step on mines
    elif board_ans[str(p1) + str(p2)] == "X":
        print("Game over \n")
        show_board(board_ans)
        GameOver = True

    # valid input
    elif board[str(p1) + str(p2)] == " ":
        coor = [[p1, p2]]
        while len(coor):
            p1 = coor[0][0]
            p2 = coor[0][1]
            if board_ans[str(p1) + str(p2)] == "0":
                for x, y in dir:
                    coor_x = p1 + x
                    coor_y = p2 + y
                    if 0 <= coor_x < 9 and 0 <= coor_y < 9:
            ...
            board[str(p1) + str(p2)] = board_ans[str(p1) + str(p2)]
            del coor[0]
        show_board(board)
...
def flag(pos, board):
    ...
    # flag
    p3 = pos[2]
    if board[str(p1) + str(p2)] == " ":
        board[str(p1) + str(p2)] = "F"
    elif board[str(p1) + str(p2)] == "F":
        board[str(p1) + str(p2)] = " "
    else:
        show_board(board)
...
def main():
    ...
    # initialize board (type = dictionary)
    board = {}
    for i in range(9):
        for j in range(9):
            pos = str(i) + str(j)
